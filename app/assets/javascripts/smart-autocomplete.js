...
(function($){
  $.fn.smartAutoComplete = function(){
    ...
    var default_filter_matcher = function(term, source, context){
                                    ...
                                    // });
                                    // var fuse = new Fuse(source, options); // "list" is the item array
                                    ...
                                    //   productStr = item.product.replace("_", " ")
                                    //   vendorStr = item.vendor.replace("_", " ")
                                    //   displays.push(productStr + " BY " + vendorStr)
                                    // })
                                    // return displays
                                    var displays = []
                                    source.forEach(function(item){
                                      productStr = item.name.replace("_", " ")
                                      vendorStr = item.vendor_name.replace("_", " ")
                                      displays.push(productStr + " BY " + vendorStr)
                                    })
                                    return displays
                                 }
        ...
        if(results_container){
          if(options.alignResultsContainer){
            results_container.css({
                  ...
                  width: function(){ return $(context).width() + 26; },
                  zIndex: 1000
            })
          }
